 ...
 * 
 * Displays a banner prompting guests to verify their email address
 * Similar to the subscription banner on the host dashboard
 */
...
export const EmailVerificationBanner = () => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'users', user.uid), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (verified && !isVerified) {
          refreshUserProfile();
          setDismissed(true);
          toast.success('Email verified successfully! Booking features are now enabled.');
        }
      }
    });
  ...
  // Don't show if user is not a guest, email is verified, or banner is dismissed
  ...
  return (
    <Card className="mb-6 border-role-guest/50 bg-role-guest/10">
      <CardContent className="pt-6">
        <div className="flex items-start gap-3">
          <AlertCircle className="h-5 w-5 text-role-guest mt-0.5 flex-shrink-0" />
          <div className="flex-1">
            <h3 className="font-semibold mb-1">Email Verification Required</h3>
            <p className="text-sm text-muted-foreground mb-3">
              Please verify your email to enable booking features. Check your inbox for a 6-digit verification code.
            </p>
