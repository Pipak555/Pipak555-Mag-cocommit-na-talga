/**
 * Email Verification Banner Component for Admins
 * 
 * Displays a banner prompting admins to verify their email address
 * Similar to the guest and host verification banners
 */
...
export const AdminEmailVerificationBanner = () => {
  ...
  // Don't show if user is not an admin, email is verified, or banner is dismissed
  if (!user || userProfile?.role !== 'admin' || isVerified || dismissed) {
    return null;
  }
  ...
  return (
    <Card className="mb-6 border-role-admin/50 bg-role-admin/10">
      <CardContent className="pt-6">
        <div className="flex items-start gap-3">
          <AlertCircle className="h-5 w-5 text-role-admin mt-0.5 flex-shrink-0" />
          <div className="flex-1">
            <h3 className="font-semibold mb-1">Email Verification Required</h3>
            <p className="text-sm text-muted-foreground mb-3">
              Please verify your email to enable all admin features. Check your inbox for a 6-digit verification code.
            </p>
